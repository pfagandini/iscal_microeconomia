...
\draw[->, thick] (0,-0.1) -- (0,4) node[left]{};
  ...
  \draw[blue!40!black, dashed, thick, domain=0.25:5, variable=\x, samples=60] plot (\x,{cfm(\x)})
    node[right]{$CFM$};
  ...
  \draw[dotted] ({25/8},0) node[below left]{\footnotesize $Q_{CVM_{min}}$}
    -- ({25/8},{cvm(25/8)});

  % mínimo CTM: cmg=ctm, maior que 25/8
  % ct = cv+1; cmg = ct'/1; ctm = ct/x; cmg=ctm -> approx x=4.2

  \draw[dotted] (3.93,0) node[below right]{\footnotesize $Q_{CTM_{min}}$}
    -- (3.93,{ctm(3.93)});
