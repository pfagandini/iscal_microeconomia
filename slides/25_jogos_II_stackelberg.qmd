...
## Da Forma Normal à Forma Extensiva {.smaller}
...
## O Jogo de Entrada no Mercado {.smaller}

Uma empresa **Entrante** pondera entrar num mercado onde existe uma empresa **Incumbente**.

. . .

```{r, engine='tikz', fig.width=6.8, fig.height=2.8, fig.align='center'}
\begin{tikzpicture}[
  grow=right,
...
\node[circle,draw,thick,minimum size=0.48cm,inner sep=1pt] {E}
  child {
    node[circle,draw,thick,minimum size=0.48cm,inner sep=1pt] {I}
      child {
        node[rectangle,draw,minimum width=1.25cm,minimum height=0.52cm,inner sep=1pt] {$(-2,\,-1)$}
        edge from parent node[above,sloped,midway,font=\tiny]{ \textit{Combater}}
      }
      child {
        node[rectangle,draw,minimum width=1.25cm,minimum height=0.52cm,inner sep=1pt] {$(3,\ 2)$}
        edge from parent node[below,sloped,midway,font=\tiny]{ \textit{Acomodar}}
      }
    edge from parent node[above,sloped,midway,font=\footnotesize]{\textit{Entrar}}
  }
  child {
    node[rectangle,draw,minimum width=1.25cm,minimum height=0.52cm,inner sep=1pt] {$(0,\ 5)$}
    edge from parent node[below,sloped,midway,font=\footnotesize]{\textit{Ficar Fora}}
  };
\end{tikzpicture}
```
...
## Solução por Indução para Trás {.smaller}
...
## Ameaças Credíveis e Não Credíveis {.smaller}
...
## Passo 3 — Equilíbrio de Stackelberg {.smaller}
