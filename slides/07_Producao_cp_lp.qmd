...
\onslide<1->{
  \draw[->, >=latex, draw=green!60!white, fill=green!20!white, line width=10pt]
    (0,0) to node[black]{$K$} (0,2);
  \draw[->, >=latex, draw=green!60!white, fill=green!20!white, line width=10pt]
    (1.5,0) to node[black]{$L$} (1.5,2);
}
\onslide<2->{
  \draw[->, >=latex, green!60!black, line width=20pt]
    (-0.5,3) to node[white]{\textbf{Inputs}} (2.5,3);
}
\onslide<3->{
  \node[draw=blue!90!white, rectangle, fill=blue!70!white, inner sep=5mm, text=white]
    at (5,3) {\Large Tecnologia};
}
\onslide<4->{
  \draw[->, >=latex, blue!60!black, line width=20pt]
    (7.5,3) to node[white]{\textbf{Output}} (10,3);
}
\end{tikzpicture}
```
...
7 & 81 & 11.57 & 9 \\
8 & 86 & 10.75 & 5 \\
9 & 78 & 8.67 & \textcolor{red}{$-8$} \\
10 & 67 & 6.70 & \textcolor{red}{$-11$} \\ \hline
\end{tabular}
```

::: {.fragment}
- Linha amarela: MPL máximo (L=4, óptimo técnico de MPL)
- Quando $MPL > APL$: APL está a crescer; quando $MPL < APL$: APL está a diminuir
:::
...
Avaliam como o *output* se altera quando os *inputs* variam na **mesma proporção** ($\alpha > 1$):

::: {.nonincremental}
- $F(\alpha K,\alpha L) > \alpha F(K,L)$ → **Rendimentos Crescentes** à Escala
  - Típicos de processos intensivos em capital (infraestruturas, redes...)
- $F(\alpha K,\alpha L) = \alpha F(K,L)$ → **Rendimentos Constantes** à Escala
  - Típicos de processos intensivos em trabalho (agricultura, artesanato...)
- $F(\alpha K,\alpha L) < \alpha F(K,L)$ → **Rendimentos Decrescentes** à Escala
:::
...
::: {.callout-note appearance="minimal"}
A função $F(K,L)$ é **homogénea de grau $i$** se:
$$F(\alpha K, \alpha L) = \alpha^i \cdot F(K,L)$$
:::

::: {.fragment}
**Exemplo:** $Q = 2KL$

$$F(\alpha K, \alpha L) = 2(\alpha K)(\alpha L) = \alpha^2 \cdot 2KL = \alpha^2 F(K,L)$$
:::

::: {.fragment}
→ Homogénea de grau 2 → **Rendimentos Crescentes** à Escala (porque $\alpha^2 > \alpha$ para $\alpha > 1$).
:::
...
b) Corresponde à Etapa III, onde o output é máximo.
c) **Corresponde à Etapa II, onde $MPL \geq 0$ e APL está a decrescer.** ✓
...
**Enunciado:** Considere a função de produção $Q = -L^3 + 30L^2 + 10L$, com $K=1$ fixo.
