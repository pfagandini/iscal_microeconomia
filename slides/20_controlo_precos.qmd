...
| Excedente do Consumidor | 8 | 10 | **+2** |
| Excedente do Produtor | 8 | 2 | **−6** |
| Excedente Total | 16 | 12 | **−4 (DWL)** |

. . .

::: {.callout-important}
Os consumidores que **conseguem comprar** ficam melhor (pagam menos). Mas o mercado serve **menos** consumidores: $Q_S = 2$ em vez de $Q^* = 4$. O excedente total cai.
:::

## Consequências não intencionais do $P_{max}$

**Escassez e racionamento não-preço**

Quem recebe o bem? Não necessariamente quem mais o valoriza. O racionamento é feito por filas, relações pessoais ou discrição do vendedor.
...
::: {.callout-note}
### Simetria notável
O DWL é igual nos dois casos (4 unidades). A redistribuição é oposta: o $P_{max}$ favorece consumidores, o $P_{min}$ favorece produtores. Em ambos os casos, o mercado serve **menos** agentes e o excedente total diminui.
:::
...
## Caso 1: Controlo de rendas — São Francisco
...
## Caso 2: Berlim — *Mietendeckel* (2020–2021)
...
## Caso 3: Catalunha — controlo de rendas 2020–2022
...
```{r fig-synth, fig.width=8.5, fig.height=3.8, out.width="95%"}
df <- data.frame(
  politica = factor(
...
## Quando podem os controlos de preços ser justificados?
...
## Solução — escolha múltipla 1 {visibility="uncounted"}
...
::: {.callout-tip}
Com $P_{max} < P^*$, a quantidade transaccionada é determinada pela oferta ($Q_S$). O EC pode aumentar para os compradores com sorte, mas o EP cai e o excedente total é sempre menor do que no equilíbrio competitivo.
:::
...
C) 10 ✗ — este é o número de trabalhadores que querem trabalhar, não o desemprego

D) 0 ✗ — o salário mínimo está acima do equilíbrio, logo tem efeito
:::
...
## Solução — desenvolvimento (a, b, c) {visibility="uncounted"}
...
$$\text{EC} = \int_0^{100}(1000 - 2Q - 400)\,dQ = \left[600Q - Q^2\right]_0^{100} = 50\,000$$

$$\text{EP} = \int_0^{100}(400 - 200 - 2Q)\,dQ = \left[200Q - Q^2\right]_0^{100} = 10\,000$$

$$\text{DWL} = \int_{100}^{200}(800 - 4Q)\,dQ = \left[800Q - 2Q^2\right]_{100}^{200} = 20\,000$$

EC + EP + DWL $= 50\,000 + 10\,000 + 20\,000 = 80\,000$ = excedente competitivo ✓

## Solução — desenvolvimento (d) {visibility="uncounted"}
