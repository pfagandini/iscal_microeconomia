	...
	\begin{center}
		\begin{tikzpicture}[
			...
			\only<4>{
				...
				\draw[dashed](\ea,0)node[below]{$q_1$}--(\ea,{d(\ea)});
				\draw[dashed](0,{s(\ea)})node[left]{$p_s$} -- (\ea,{s(\ea)});
				\draw[dashed](0,{d(\ea)})node[left]{$p_d$} -- (\ea,{d(\ea)});
			}
...
\begin{frame}
	...
	\begin{center}
		\begin{tikzpicture}[
			...
			\draw[dashed](\ea,{s(\ea)}) -- (\ea,0)node[below]{$q_d$};
			\draw[dashed](\eb,{d(\eb)}) -- (\eb,0)node[below]{$q_s$};
